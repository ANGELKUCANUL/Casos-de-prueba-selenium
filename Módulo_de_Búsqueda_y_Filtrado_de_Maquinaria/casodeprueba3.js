...
(async function testBusquedaPorTipoMaquinariaGrande() {
    ...
    try {
        ...
        let cuadroTextoPrecio = await driver.findElement(By.id("ubicacion")); 
        await cuadroTextoPrecio.clear(); 
        await cuadroTextoPrecio.sendKeys("merida"); 
        ...
        await driver.wait(until.urlContains('http://localhost:3000/Maquinariafiltro?ubicacion=merida'), 5000);
        let currentUrl = await driver.getCurrentUrl(); // Obtener la URL actual
        if (currentUrl === 'http://localhost:3000/Maquinariafiltro?ubicacion=merida') {
