...
(async function testBusquedaPorTipoMaquinariaGrande() {
    ...
    try {
        ...
        let cuadroTextoPrecio = await driver.findElement(By.id("disponibilidad")); 
        await cuadroTextoPrecio.clear(); 
        await cuadroTextoPrecio.sendKeys("5"); 
        ...
        await driver.wait(until.urlContains('http://localhost:3000/Maquinariadisponible?disponibilidad=5'), 5000);
        let currentUrl = await driver.getCurrentUrl(); // Obtener la URL actual
        if (currentUrl === 'http://localhost:3000/Maquinariadisponible?disponibilidad=5') {
            console.log("Caso de prueba exitoso: Se redirigió correctamente a la URL esperada.");
        } else {
